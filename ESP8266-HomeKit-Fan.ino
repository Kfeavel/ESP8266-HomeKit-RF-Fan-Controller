...
static void cha_fan_on_setter(const homekit_value_t value) {
  ...
  bool on = value.bool_value;
  if (cha_fan_active.value.bool_value != on) {
...
static homekit_value_t cha_fan_on_getter() {
  LOG_D("[GET] Fan on: %s", (cha_fan_active.value.bool_value ? "ON" : "OFF"));
  return cha_fan_active.value;
}
...
static void my_homekit_setup() {
  ...
  cha_fan_active.getter = cha_fan_on_getter;
